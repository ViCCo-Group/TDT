...
cfg.results.overwrite = 0; % should not overwrite results (change if you whish to do so)

cfg.plot_selected_voxels = 1000; % show SL every 1000' steps

n_perms = 10;  % normally you should pick large number here
                 % the function might compute less if less permutations are available
combine = 0;
designs = make_design_permutation(cfg,n_perms,combine);


%% Run all permutations in a loop
for i_perm = 1:n_perms-1
    ...
    % rename all design figures to start with the current permutation
    % number
