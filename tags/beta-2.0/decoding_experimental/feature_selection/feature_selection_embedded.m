...
elseif strcmp(n_vox,'automatic')
    ...
    [n_vox_selected,output,cfg.feature_selection.design.msg] = run_nest(cfg,data,i_step,n_vox,nested_n_vox); % determine optimal number of features
...
for i_step = 1:n_steps % loop over decoding steps (e.g. runs) within training data
    ...
    for iteration = 1:length(nested_n_vox)
        ...
        decoding_out = feval(fhandle,labels_test,vectors_test,fs_cfg,model);
