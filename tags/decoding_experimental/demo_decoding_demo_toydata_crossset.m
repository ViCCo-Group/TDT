...
%% end rep
% acc(n_rep) =  results.accuracy % collect the results
% end % end for n_rep
...
if size(data, 2) == 2
    ...
    for i_model = 1:length(results.model_parameters.output(1).model)
        model = results.model_parameters.output(1).model(i_model);
