function check_inputdata_transformations(caller_function_name, cfg)

% Check that no transformation (e.g. scaling, feature_transfomration) of
...
% cfg.scale.CHECK_data_transformation_for_model_transres_ok = true
% We recommend to use this checkc in all tranformations that in some way 
% on the model paramters.
%
...
% CHECK_data_transformation_for_model_transres_ok, if you need to do so.
...
for cf_ind = 1:length(check_fields)
    try % report a different error if the field does not exist
        if ~strcmp(cfg.(check_fields{cf_ind}).method, 'none')
            if ~isfield(cfg.(check_fields{cf_ind}), 'CHECK_data_transformation_for_model_transres_ok') || ...
                ~cfg.(check_fields{cf_ind}).CHECK_data_transformation_for_model_transres_ok % of if set to false
                ...
                error_string = [error_string caller_function_name ' creates an output that depends on the model parameter, but the input data is transformed by cfg.' check_fields{cf_ind} '.method=''' cfg.(check_fields{cf_ind}).method '''.' char(10) ...
                    ...
                    '   cfg.' check_fields{cf_ind} '.CHECK_data_transformation_for_model_transres_ok = true' char(10) char(10)];
            
            else
                % check has been acknowledged by the user
                warningv('check_inputdata_transformation:data_transformation_ok', ...
                    [error_string caller_function_name ' creates an output that depends on the model parameter, but the input data is transformed by cfg.' check_fields{cf_ind} '.method=''' cfg.(check_fields{cf_ind}).method '''.' char(10) ...
                     'This is ok, because it has been acknowledge that by the user: cfg.' check_fields{cf_ind} '.CHECK_data_transformation_for_model_transres_ok = true'])
            end
        end
    catch e
