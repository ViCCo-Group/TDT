...
if strcmp(cfg.decoding.software, 'libsvm') && strcmp(cfg.scale.method, 'none')
    if ~isfield(cfg.scale, 'IKnowThatLibsvmCanBeSlowWithoutScaling')
        error('decoding_basic_checks:setting_default_scaling_for_libsvm', ...
           ['NOTE: This is a new warning introduced in TDT version 3.999E 2021/09/09. ' ...
            'PROBLEM: Libsvm can be VERY slow for some datasets (e.g. some subjects or folds) when data is not scaled. ' ...
            'SOLUTION: To prevent this, employ a scaling method. libsvm recommends ''min0max1'' for ''all'' data.'...
            'To do so, add the following before you call decoding(cfg): cfg.scale.method = ''min0max1''; cfg.scale.estimation = ''all''; (For more, see help decoding_scale_data). ' ...
