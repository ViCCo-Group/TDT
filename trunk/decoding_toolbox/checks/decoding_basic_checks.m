    ...
    if ~isfield(cfg.scale, 'IKnowThatLibsvmCanBeSlowWithoutScaling')
        error('decoding_basic_checks:setting_default_scaling_for_libsvm', ...
           ['Libsvm can be VERY slow for some datasets (e.g. some subjects or folds) when data is not scaled. ' ...
            ...
            'To avoid this error, set e.g. cfg.scale.method = ''min0max1'' and cfg.scale.estimation = ''all'' (see help decoding_scale_data). ' ...
            'NOTE: If you have many old scripts, change this error to a warning, and everything will work as it used to.'])
    end
end
